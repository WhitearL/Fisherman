...
using MotionDetector = AForge.Vision.Motion.MotionDetector;
...
namespace WowFishingAssist
{
    public partial class MainForm : Form
    {
        ...
        private readonly Timer globalTimer          = new System.Timers.Timer() { Interval = 30000, Enabled = false, AutoReset = true };   // 30 Seconds
        private readonly Timer lureTimer            = new System.Timers.Timer() { Interval = 900000, Enabled = false, AutoReset = true };  // 15 minutes
        private readonly Timer baitTimer            = new System.Timers.Timer() { Interval = 1800000, Enabled = false, AutoReset = true }; // 30 minutes
        private FormsTimer screenUpdateTimer        = new System.Windows.Forms.Timer() { Interval = 1000, Enabled = true };
        private ScreenStateLogger screenStateLogger = new ScreenStateLogger();
        private MotionDetector motionDetector       = GetDefaultMotionDetector();
        private int warmupTime                      = 5000 * 10000;
        private long warmupStartTime                = DateTime.Now.Ticks + (10000 * 100000);
        private bool inWarmupMode                   = true;
        private bool running                        = false;
        private int origScreenHeight                = 0;
        private int origScreenWidth                 = 0;
        ...
        private async Task sendUseBait()
        {
            ...
            // Seems bait has to be tied to an action key.
        ...
        private async void DoMouseClick(Point p)
        {
        ...
        private async void ClickBobber(Rectangle x1)
        {
            ...
            // Get the biggest bounding box and set it to move the mouse to the center of bounding box. 
        ...
        {
            // Delay 10 seconds before we start scanning anything.  This way the background of the image can be learned.
            warmupStartTime = DateTime.Now.Ticks + (10000 * 100000);
            globalTimer.Stop();
            inWarmupMode = true;
            running = false;
        }
        ...
        private void buStartStop_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SetForegroundWindow(hWnd);    // bring Wow into the forground.
                if (cbUseBait.Checked) useBait = true;
                ResumeFishing();
            }
        }
        ...
        private void buApplySettings_Click(object sender, EventArgs e)
        {
            ...
            motionDetector = x;
            mouseClickTime = (int)numTimeToHoldClick.Value;
        }
